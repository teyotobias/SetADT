...
void checkCase(setFun fun, Set* s1, Set* s2, Set* expect) {
    ...
    (*fun)(&res, s2);
    assert(isEqualToSet(&res, expect)); //THIS ASSERT FAILS!! FIX
    destroySet(&res);
}
...
void specialCaseTests(void) {
    ...
    checkCase(&intersectFromSet, &universal, &empty, &empty); //error here
